...
function signIn($email, $password) {
  if (!$email || !$password) {
    ...
    $emailTest = preg_match($emailRegex, $email);
    if ($emailTest === 1) {
      session_start();
      // session_destroy();
...
function logIn($email, $password) {
  if (!$email || !$password) {
    ...
    if ($emailTest === 1) {
      session_start();
      if ($email === $_SESSION['email'] && md5($password) === $_SESSION['password']) {
        echo 'Bienvenue';
      } else {
        echo "tant pis";
        session_abort();
...
if ($_POST['action'] == 'add') {
  ...
  removeFromCart($_POST['id'], $_POST['price']);
} else if ($_POST['action'] == 'clear') {
  clearCart();
} else if ($_POST['action'] == 'sign') {
  logIn($_POST['email'], $_POST['pwd']);
} else if ($_POST['action'] == 'log') {
  signIn($_POST['email'], $_POST['pwd']);
}